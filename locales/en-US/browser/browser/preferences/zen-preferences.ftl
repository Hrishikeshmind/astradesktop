...
zen-performance-description = Status and tools for memory, battery, and processes. Values come from the browser engine — Astra does not invent per-tab RAM figures.
...
zen-performance-smart-suspend-status = Smart Suspend: Controlled in Tab Management above (the browser also unloads tabs under memory pressure).
zen-performance-session-restore-status = Session restore: Background tabs restore lazily; pinned tabs restore on demand.
zen-performance-hw-accel-status = Hardware acceleration: Automatic (engine / driver policy)
zen-performance-process-isolation-status = Process isolation: Engine-managed (Fission / site isolation)
...
zen-local-translation-description = Translation happens on this device after the required language model is downloaded. Not every language is supported. Astra does not send page contents to Astra servers.
...
zen-theme-marketplace-description = Import, export, and update installed mods. A mods store is not available yet.
