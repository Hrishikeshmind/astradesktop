# ASTRA-CAP-047 — Autoplay controls
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 47

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-047` |
| Slug | `autoplay-controls` |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 47). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Restrictive autoplay default may over-block wanted playback; verify.
...
| canonical_index | 47 |
| canonical_title | Autoplay controls |
...
| notes | Canonical slice of matrix block F2 (autoplay policy). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Gecko AutoplayPolicy |
...
- `prefs/firefox/browser.yaml (media.autoplay.default verified)`
...
`NATIVE_PREF` `media.autoplay.default` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
