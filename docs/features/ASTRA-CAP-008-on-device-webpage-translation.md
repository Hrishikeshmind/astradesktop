# ASTRA-CAP-008 — On-device webpage translation
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 8

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-008` |
| Slug | `on-device-webpage-translation` |
| Title | On-device webpage translation |
| Canonical group | Student & Reading |
| Secondary groups | `Language & Access`, `India & Language` |
| Audiences | `STU`, `NORM`, `GOV`, `CORP` |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 8). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- On-device Bergamot; do not build a second translator.
...
| canonical_index | 8 |
| canonical_title | On-device webpage translation |
...
| notes | Canonical slice of matrix block A3 (Translations engine). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox Toolkit (Bergamot on-device) |
| UX owner | Astra entrypoint over native Translations |
...
Proposed translation discoverability entrypoint over native urlbar translate affordance / about:translations.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/firefox/multilingual.yaml (verified in worktree)`
