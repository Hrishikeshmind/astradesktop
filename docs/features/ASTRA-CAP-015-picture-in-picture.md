...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 15

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-015` |
| Slug | `picture-in-picture` |
| Title | Picture-in-Picture |
| Canonical group | Media & Classes |
| Secondary groups | `Entertainment` |
| Audiences | `STU`, `ENT`, `NORM` |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 15). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Playing/PiP tabs must be exempt from tab suspension/savers.
...
| canonical_index | 15 |
| canonical_title | Picture-in-Picture |
...
| notes | 1:1 with matrix block A9 (Picture-in-Picture). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox Toolkit (PiP) |
| UX owner | Astra urlbar PiP button over native PiP |
...
- `engine/toolkit/components/pictureinpicture/`

## Proposed Astra / Zen entrypoints

Urlbar PiP button preference already present; native PiP remains backend.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/zen/zen-urlbar.yaml (zen.urlbar.show-pip-button verified)`
- `prefs/firefox/pip.yaml (present)`
...
`NATIVE_PREF` `zen.urlbar.show-pip-button` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `zen.urlbar.show-pip-button` and/or remove Astra discoverability surface. Do not delete native backend.
