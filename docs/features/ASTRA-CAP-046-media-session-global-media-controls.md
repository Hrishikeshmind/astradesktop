# ASTRA-CAP-046 — Media Session / global media controls
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 46

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-046` |
| Slug | `media-session-global-media-controls` |
| Title | Media Session / global media controls |
...
Primary classification `ENABLE_AFTER_TEST` is taken exactly from the human-approved 62-item catalog (index 46). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- zen.mediacontrols off by default; enable only after test.
...
| canonical_index | 46 |
| canonical_title | Media Session / global media controls |
...
| notes | Canonical slice of matrix block F2 (Media Session/Zen controls). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Gecko mediacontrol + Zen ZenMediaController |
| UX owner | Zen/Astra media controls UI |
...
- `engine/dom/media/mediacontrol/`

## Proposed Astra / Zen entrypoints

Zen media controls UI candidate; off by default until ENABLE_AFTER_TEST passes.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/media/ZenMediaController.mjs (present)`
- `prefs/zen/zen.yaml (zen.mediacontrols.enabled verified)`

## State

| Field | Value |
|---|---|
| state_status | `IN_MEMORY_ONLY` |
| state_refs | `ASTRA-STATE-016` |

- `ASTRA-STATE-016` — owner: Gecko mediacontrol + Zen ZenMediaController (in-memory) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
...
- `ASTRA-CONFLICT-013` — RAM Saver vs audio/video tabs (map #13, S2). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
`NATIVE_PREF` `zen.mediacontrols.enabled` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `zen.mediacontrols.enabled` and/or remove Astra discoverability surface. Do not delete native backend.
