# ASTRA-CAP-058 — Site permissions and clear-site-data controls
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 58

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-058` |
| Slug | `site-permissions-and-clear-site-data` |
| Title | Site permissions and clear-site-data controls |
| Canonical group | Privacy & Security |
| Secondary groups | _none_ |
| Audiences | `NORM`, `CORP`, `GOV` |
| Classification | `INTEGRATE` |
| Supporting classifications | `EXPOSE_NATIVE`, `USE_NATIVE` |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 58). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- sanitizeOnShutdown default may surprise users; reconsider per persona.
...
| canonical_index | 58 |
| canonical_title | Site permissions and clear-site-data controls |
...
| notes | Canonical slice of matrix blocks E5 (normal-user controls) and G1 (permissions). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox permissions + sanitize |
| UX owner | Astra/Suraksha surface over native permissions |
...
- `Firefox permissions + sanitize`

## Proposed Astra / Zen entrypoints

Suraksha Permissions/SiteData adapters deep-link to native surfaces.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/common/modules/AstraSurakshaPermissions.mjs (present)`
- `src/zen/common/modules/AstraSurakshaSiteData.mjs (present)`
...
- `ASTRA-CONFLICT-023` — Private windows vs persistent App Hub / Space state (map #23, S2). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
