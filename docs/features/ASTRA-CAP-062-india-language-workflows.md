# ASTRA-CAP-062 — Hindi/Indian-language UI, dictionaries and local translation workflows
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 62

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-062` |
| Slug | `india-language-workflows` |
| Title | Hindi/Indian-language UI, dictionaries and local translation workflows |
| Canonical group | India & Language |
| Secondary groups | `Language & Access` |
| Audiences | `NORM`, `STU`, `GOV` |
| Classification | `INTEGRATE` |
| Supporting classifications | `USE_NATIVE`, `EXPOSE_NATIVE` |
...
| Dependencies | `ASTRA-CAP-008`, `ASTRA-CAP-017` |

### Why this classification

Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 62). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Verify hi langpack + Indian-language dictionaries actually packaged.
...
| canonical_index | 62 |
| canonical_title | Hindi/Indian-language UI, dictionaries and local translation workflows |
| canonical_group | India & Language |
...
| notes | Cross-cutting: A3 (translation), A11 (dictionaries), Part D (Indian-language support). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox l10n/translations + Hunspell + Astra India catalog |
| UX owner | Astra India integration |
...
- `engine/extensions/spellcheck/`
- `locales/**`

## Proposed Astra / Zen entrypoints

India catalog / localization prioritization integration over native l10n + translations + dictionaries.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/firefox/india.yaml (intl.accept_languages, browser.search.region verified)`
...
| state_refs | `ASTRA-STATE-028`, `ASTRA-STATE-007`, `ASTRA-STATE-026` |

- `ASTRA-STATE-028` — owner: Firefox l10n + prefs (`intl.accept_languages`, `browser.search.region`) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
...
- `ASTRA-STATE-026` — owner: Firefox Editor (Hunspell) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))

## Conflicts and resolution rules

- `ASTRA-CONFLICT-020` — Fluent localization vs nested toolbar DOM (map #20, S3). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
`NATIVE_PREF` `intl.accept_languages` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `intl.accept_languages` and/or remove Astra discoverability surface. Do not delete native backend.
