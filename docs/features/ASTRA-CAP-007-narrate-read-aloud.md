...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 7

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-007` |
| Slug | `narrate-read-aloud` |
...
Primary classification `ASTRA_UX_WRAPPER` is taken exactly from the human-approved 62-item catalog (index 7). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Astra command cmd_zenReadAloud wraps native Narrate via AstraPhase1Actions.
...
| canonical_index | 7 |
| canonical_title | Narrate / Read Aloud |
...
| notes | Canonical slice of matrix block A2 (Narrate/Read-Aloud); Astra wrapper exists. |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox Toolkit (Narrate) |
| UX owner | Astra (AstraPhase1Actions wrapper) |
...
- `engine/toolkit/components/narrate/`

## Proposed Astra / Zen entrypoints

Existing Astra command cmd_zenReadAloud via AstraPhase1Actions (audit-documented; verify wiring at E2 later).

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/common/modules/AstraPhase1Actions.mjs (present in worktree)`
...
| state_refs | `ASTRA-STATE-006` |

- `ASTRA-STATE-006` — owner: Firefox Reader/Narrate (transient) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
