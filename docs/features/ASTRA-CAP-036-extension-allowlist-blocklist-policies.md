# ASTRA-CAP-036 — Extension allowlist/blocklist and install policies
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 36

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-036` |
| Slug | `extension-allowlist-blocklist-policies` |
| Title | Extension allowlist/blocklist and install policies |
| Canonical group | Corporate |
| Secondary groups | `Extensions` |
| Audiences | `CORP`, `GOV` |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 36). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Delivered via ExtensionSettings policy; signed-only install retained.
...
| canonical_index | 36 |
| canonical_title | Extension allowlist/blocklist and install policies |
...
| notes | Sub-aspect of matrix block C1 (policy engine) and G6 (extensions). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox EnterprisePolicies + add-ons |
| UX owner | Astra managed integration over native policy |
...
- `engine/browser/components/enterprisepolicies/`
- `engine/toolkit/mozapps/extensions/`

## Proposed Astra / Zen entrypoints

Proposed managed integration for ExtensionSettings; Suraksha uBlock adapter is read-oriented.
...
| state_refs | `ASTRA-STATE-020`, `ASTRA-STATE-017` |
...
`POLICY_CONTROLLED` `ExtensionSettings` (status: PROPOSED).
...
Use enterprise policy controls (`ExtensionSettings`) and/or remove Astra status/entrypoint surface.
