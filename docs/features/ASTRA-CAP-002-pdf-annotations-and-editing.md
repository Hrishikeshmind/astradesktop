# ASTRA-CAP-002 — PDF annotations and editing tools
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 2

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-002` |
| Slug | `pdf-annotations-and-editing` |
| Title | PDF annotations and editing tools |
| Canonical group | Student & Reading |
| Secondary groups | _none_ |
| Audiences | `STU`, `NORM`, `CORP`, `GOV` |
...
| Dependencies | `ASTRA-CAP-001` |

### Why this classification

Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 2). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.
...
| canonical_index | 2 |
| canonical_title | PDF annotations and editing tools |
...
| notes | Canonical slice of matrix block A1 (highlight/annotation editor). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox Toolkit (pdf.js) |
| UX owner | Astra entrypoint over native editor |
...
- `engine/toolkit/components/pdfjs/`

## Proposed Astra / Zen entrypoints

Proposed student/PDF annotation discoverability entrypoint (menu/command) over native pdf.js editor; no second PDF stack.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/firefox/pdf.yaml (verified in worktree: pdfjs.enableHighlightEditor)`

## State

| Field | Value |
|---|---|
| state_status | `NATIVE_STATELESS` |
| state_refs | `ASTRA-STATE-008` |

- `ASTRA-STATE-008` — owner: Firefox pdf.js (in-document only) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
...
`NATIVE_PREF` `pdfjs.enableHighlightEditor` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `pdfjs.enableHighlightEditor` and/or remove Astra discoverability surface. Do not delete native backend.
