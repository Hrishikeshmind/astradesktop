# ASTRA-CAP-041 — Managed-browser / about:policies status
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 41

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-041` |
| Slug | `managed-browser-about-policies-status` |
| Title | Managed-browser / about:policies status |
...
Primary classification `ASTRA_UX_WRAPPER` is taken exactly from the human-approved 62-item catalog (index 41). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Read-only managed status panel; proposed flag from build-trains.
...
| canonical_index | 41 |
| canonical_title | Managed-browser / about:policies status |
...
| notes | 1:1 with matrix block C5 (managed indicator/diagnostics). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox (about:policies) + policy engine |
| UX owner | Astra managed-status panel (to build) |
...
- `engine/browser/components/enterprisepolicies/ (about:policies)`

## Proposed Astra / Zen entrypoints

Proposed Managed-Browser / about:policies status panel (lazy + static fallback).
...
`ASTRA_PREF_NEW` proposed `astra.managed.status.enabled` (status: PROPOSED) — not created in this pass.
...
Flip/disable proposed `astra.managed.status.enabled` (when created in a later implementation pass) and/or remove the Astra entrypoint/panel. Native backend remains.
