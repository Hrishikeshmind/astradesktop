# ASTRA-CAP-060 — Native updater, troubleshooting and recovery foundations
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 60

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-060` |
| Slug | `native-updater-troubleshooting-recovery` |
| Title | Native updater, troubleshooting and recovery foundations |
| Canonical group | Reliability |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 60). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- GOV update signing DEFERRED (ASTRA-EXCLUSION-007); crash reporter not shipped.
...
| canonical_index | 60 |
| canonical_title | Native updater, troubleshooting and recovery foundations |
| canonical_group | Reliability |
...
| notes | Canonical slice of matrix block G7 (updates/recovery) and C4 (update service). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Toolkit updater + troubleshooting/reset |
| UX owner | Astra integration over native update/recovery |
...
- `troubleshooting/reset/about:support`

## Proposed Astra / Zen entrypoints

ZenUpdates / UpdateUtils patch coordination; signed-update hardening deferred.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/common/modules/ZenUpdates.mjs (present)`
- `prefs/zen/updates.yaml (verified)`
...
| state_refs | `ASTRA-STATE-018`, `ASTRA-STATE-003` |
...
`NATIVE_PREF` `astra.updates.show-update-notification` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `astra.updates.show-update-notification` and/or remove Astra discoverability surface. Do not delete native backend.
