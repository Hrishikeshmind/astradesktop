# ASTRA-CAP-001 — PDF viewing and navigation
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 1

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-001` |
| Slug | `pdf-viewing-and-navigation` |
| Title | PDF viewing and navigation |
| Canonical group | Student & Reading |
| Secondary groups | _none_ |
| Audiences | `STU`, `NORM`, `CORP`, `GOV` |
| Classification | `USE_NATIVE` |
| Supporting classifications | `EXPOSE_NATIVE` |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 1). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.
...
| canonical_index | 1 |
| canonical_title | PDF viewing and navigation |
...
| notes | Canonical slice of matrix block A1 (PDF.js viewer). |
...
None required (NATIVE_ONLY). Reachability via opening a PDF in a verified Astra installer.
