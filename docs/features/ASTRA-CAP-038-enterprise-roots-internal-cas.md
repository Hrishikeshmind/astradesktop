# ASTRA-CAP-038 — Enterprise roots and internal certificate authorities
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 38

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-038` |
| Slug | `enterprise-roots-internal-cas` |
| Title | Enterprise roots and internal certificate authorities |
...
Primary classification `ASTRA_UX_WRAPPER` is taken exactly from the human-approved 62-item catalog (index 38). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Certificate visibility surface for managed/GOV; backend is NSS.
...
| canonical_index | 38 |
| canonical_title | Enterprise roots and internal certificate authorities |
...
| notes | Sub-aspect of matrix block C3 (enterprise roots/certs). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox/NSS |
| UX owner | Astra certificate visibility over native NSS |
...
- `engine/security/manager/ssl/`

## Proposed Astra / Zen entrypoints

Proposed certificate visibility UX over native NSS / Certificates policy.
...
`POLICY_CONTROLLED` `Certificates` (status: PROPOSED).
...
Use enterprise policy controls (`Certificates`) and/or remove Astra status/entrypoint surface.
