# ASTRA-CAP-042 — Offline/silent deployment foundation
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 42

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-042` |
| Slug | `offline-silent-deployment-foundation` |
| Title | Offline/silent deployment foundation |
...
Primary classification `ENABLE_AFTER_TEST` is taken exactly from the human-approved 62-item catalog (index 42). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Requires installer packaging; offline builds lack runtime CDM/models.
...
| canonical_index | 42 |
| canonical_title | Offline/silent deployment foundation |
...
| notes | Part D (offline installer) + C1 (policy); requires installer build to verify. |

## Owners

| Role | Owner |
|---|---|
| Native owner | NSIS installer + policy engine |
| UX owner | Astra deployment coordination |
...
- `src/browser/installer/windows/nsis/`
- `EnterprisePolicies`

## Proposed Astra / Zen entrypoints

Proposed offline/silent deployment foundation docs + installer packaging verification.
...
`POLICY_CONTROLLED` (unnamed policy surface) (status: PROPOSED).
...
Use enterprise policy controls and/or remove Astra status/entrypoint surface.
