# ASTRA-CAP-043 — Widevine / EME protected playback
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 43

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-043` |
| Slug | `widevine-eme-protected-playback` |
| Title | Widevine / EME protected playback |
...
| Dependencies | `ASTRA-CAP-044` |

### Why this classification

Primary classification `ENABLE_AFTER_TEST` is taken exactly from the human-approved 62-item catalog (index 43). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- REJECT any second DRM stack (ASTRA-EXCLUSION-005); DEFER hard DRM claims (ASTRA-EXCLUSION-009).
...
| canonical_index | 43 |
| canonical_title | Widevine / EME protected playback |
...
| notes | 1:1 with matrix block F1 (Widevine/EME/DRM). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Gecko EME + GMP (Widevine, runtime download) |
| UX owner | Astra Media Readiness (truthful status) |
...
- `engine/dom/media/eme/`
- `engine/dom/media/gmp/`

## Proposed Astra / Zen entrypoints

Proposed Media Readiness truthful DRM status surface; no second DRM stack.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/zen/media.yaml (media.eme.enabled verified)`
...
`NATIVE_PREF` `media.eme.enabled` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `media.eme.enabled` and/or remove Astra discoverability surface. Do not delete native backend.
...
- Do not claim Netflix certified / guaranteed 4K / all services / casting.
