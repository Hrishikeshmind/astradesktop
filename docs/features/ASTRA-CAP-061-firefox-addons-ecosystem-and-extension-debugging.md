# ASTRA-CAP-061 — Firefox add-ons ecosystem and extension debugging
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 61

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-061` |
| Slug | `firefox-addons-ecosystem-and-extension-debugging` |
| Title | Firefox add-ons ecosystem and extension debugging |
| Canonical group | Extensions |
| Secondary groups | `Developer` |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 61). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Signed-only install; uBlock full power; recommendations disabled.
...
| canonical_index | 61 |
| canonical_title | Firefox add-ons ecosystem and extension debugging |
| canonical_group | Extensions |
...
| notes | 1:1 with matrix block G6 (Extensions/add-ons). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox add-ons (AMO/XPInstall) |
| UX owner | Native (Firefox); Suraksha uBlock adapter |
...
- `engine/toolkit/mozapps/extensions/`

## Proposed Astra / Zen entrypoints

None required for add-ons backend (NATIVE_ONLY); Suraksha uBlock adapter surfaces status.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/firefox/extensions.yaml (xpinstall.signatures.required verified)`
- `src/zen/common/modules/AstraSurakshaUBlock.mjs (present)`
...
| state_refs | `ASTRA-STATE-020` |

- `ASTRA-STATE-020` — owner: Firefox add-ons (`extensions.json`) + EnterprisePolicies (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
...
`NATIVE_PREF` `xpinstall.signatures.required` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
