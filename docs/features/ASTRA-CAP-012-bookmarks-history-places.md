...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 12

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-012` |
| Slug | `bookmarks-history-places` |
| Title | Bookmarks, history and Places |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 12). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- REJECT any second bookmark system (ASTRA-EXCLUSION-001); Places is canonical.
...
| canonical_index | 12 |
| canonical_title | Bookmarks, history and Places |
...
| notes | 1:1 with matrix block A6 (Bookmarks/Places). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox/Places |
| UX owner | Native (Firefox) + Zen workspace bookmark mapping |
...
- `engine/browser/components/places/`
- `engine/toolkit/components/places/`

## Proposed Astra / Zen entrypoints

None required for Places backend (NATIVE_ONLY). Zen workspace-bookmark mapping is additive, not a second store.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/spaces/ZenSpaceBookmarksStorage.js (present)`
...
| state_refs | `ASTRA-STATE-001`, `ASTRA-STATE-004` |
