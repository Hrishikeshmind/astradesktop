# ASTRA-CAP-045 — Hardware acceleration and video decode status
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 45

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-045` |
| Slug | `hardware-acceleration-video-decode-status` |
| Title | Hardware acceleration and video decode status |
...
Primary classification `ASTRA_UX_WRAPPER` is taken exactly from the human-approved 62-item catalog (index 45). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Cannot claim guaranteed HW-accel until per-GPU tested.
...
| canonical_index | 45 |
| canonical_title | Hardware acceleration and video decode status |
...
| notes | Canonical slice of matrix block F2 (HW decode). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Gecko media platforms |
| UX owner | Astra Media Readiness status over native decode |
...
Proposed Media Readiness HW-decode status surface.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/firefox/performance.yaml (media.hardware-video-decoding.enabled verified)`
...
- `ASTRA-CONFLICT-015` — Transparent Mode vs hardware acceleration (map #15, S3). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
`NATIVE_PREF` `media.hardware-video-decoding.enabled` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `media.hardware-video-decoding.enabled` and/or remove Astra discoverability surface. Do not delete native backend.
