...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 30

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-030` |
| Slug | `devtools-browser-console` |
...
Primary classification `ENABLE_AFTER_TEST` is taken exactly from the human-approved 62-item catalog (index 30). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Off by default via devtools.chrome.enabled; policy-controllable.
...
| canonical_index | 30 |
| canonical_title | Browser Console |
...
| notes | Canonical slice of matrix block B2 (chrome-level console). |
...
| UX owner | Astra entrypoint over native chrome console |
