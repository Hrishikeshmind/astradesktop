# ASTRA-CAP-013 — Open-tab search / urlbar TABS mode
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 13

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-013` |
| Slug | `open-tab-search-urlbar-tabs` |
| Title | Open-tab search / urlbar TABS mode |
| Canonical group | Everyday Productivity |
| Secondary groups | _none_ |
| Audiences | `STU`, `DEV`, `NORM` |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 13). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Astra command cmd_zenSearchOpenTabs plus native % restrict mode.
...
| canonical_index | 13 |
| canonical_title | Open-tab search / urlbar TABS mode |
...
| notes | 1:1 with matrix block A7 (Tab search). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox (urlbar providers) |
| UX owner | Astra command over native urlbar |
...
- `engine/browser/components/urlbar/UrlbarProviderOpenTabs.sys.mjs`

## Proposed Astra / Zen entrypoints

Existing Astra command cmd_zenSearchOpenTabs + native % restrict mode (audit-documented).

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/common/modules/AstraPhase1Actions.mjs (present)`
- `prefs/firefox/urlbar.yaml (browser.urlbar.suggest.openpage verified)`
...
- `ASTRA-CONFLICT-001` — Firefox sidebar vs Zen vertical tabs (map #1, S2). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
`NATIVE_PREF` `browser.urlbar.suggest.openpage` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `browser.urlbar.suggest.openpage` and/or remove Astra discoverability surface. Do not delete native backend.
