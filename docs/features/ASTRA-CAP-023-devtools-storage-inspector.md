# ASTRA-CAP-023 — Storage Inspector
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 23

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-023` |
| Slug | `devtools-storage-inspector` |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 23). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.
...
| canonical_index | 23 |
| canonical_title | Storage Inspector |
...
| notes | Canonical slice of matrix block B1 (DevTools storage). |
...
Proposed Developer Hub launcher to native Storage Inspector.
