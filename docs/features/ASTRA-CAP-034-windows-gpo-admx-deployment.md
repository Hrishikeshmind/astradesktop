# ASTRA-CAP-034 — Windows GPO / ADMX deployment support
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 34

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-034` |
| Slug | `windows-gpo-admx-deployment` |
| Title | Windows GPO / ADMX deployment support |
...
Primary classification `ASTRA_UX_WRAPPER` is taken exactly from the human-approved 62-item catalog (index 34). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- ADMX templates are NOT in the tree; Astra-branded ADMX must be shipped.
...
| canonical_index | 34 |
| canonical_title | Windows GPO / ADMX deployment support |
...
| notes | 1:1 with matrix block C2 (ADMX/GPO); templates absent -> needs clarification. |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox (WindowsGPOParser) |
| UX owner | Astra (ship/brand ADMX + docs) |
...
- `engine/browser/components/enterprisepolicies/WindowsGPOParser.sys.mjs`

## Proposed Astra / Zen entrypoints

Proposed Astra-branded ADMX/ADML packaging + admin docs (templates currently absent).
