...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 6

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-006` |
| Slug | `reader-mode` |
| Title | Reader Mode |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 6). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.
...
| notes | Canonical slice of matrix block A2 (Reader Mode). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox Toolkit (Reader) |
| UX owner | Astra entrypoint over native Reader |
...
- `engine/toolkit/components/reader/`

## Proposed Astra / Zen entrypoints

Proposed Reader Mode discoverability entrypoint (native View:ReaderView / urlbar icon).
