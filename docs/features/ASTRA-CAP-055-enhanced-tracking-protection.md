# ASTRA-CAP-055 — Enhanced Tracking Protection
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 55

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-055` |
| Slug | `enhanced-tracking-protection` |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 55). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Surfaced by Suraksha (read/link only); do not weaken backend.
...
| canonical_index | 55 |
| canonical_title | Enhanced Tracking Protection |
...
| notes | Canonical slice of matrix block G1 (privacy/security stack, ETP). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox antitracking (ETP) |
| UX owner | Native (Firefox); Suraksha read-only surface |
...
- `engine/toolkit/components/antitracking/`

## Proposed Astra / Zen entrypoints

Suraksha Protection adapter reads native ETP (no write ownership).

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/common/modules/AstraSurakshaProtection.mjs (present)`
- `prefs/firefox/browser.yaml (browser.contentblocking.category verified)`
...
| state_refs | `ASTRA-STATE-013` |

- `ASTRA-STATE-013` — owner: Firefox privacy/antitracking (prefs) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))

## Conflicts and resolution rules

- `ASTRA-CONFLICT-008` — Suraksha vs native protections UI (map #8, S2). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
- `ASTRA-CONFLICT-009` — uBlock vs Enhanced Tracking Protection (map #9, S2). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
- `ASTRA-CONFLICT-010` — uBlock/ETP vs DRM, banking, authentication sites (map #10, S1). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
`NATIVE_PREF` `browser.contentblocking.category` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
