...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 19

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-019` |
| Slug | `devtools-page-inspector` |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 19). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- REJECT any second DevTools stack (ASTRA-EXCLUSION-002); Astra Inspector remap C to L.
...
| canonical_index | 19 |
| canonical_title | Page Inspector |
...
| notes | Canonical slice of matrix block B1 (DevTools toolbox). |
...
Proposed Developer Hub launcher to native Inspector; Zen KBS remap C→L (audit).
