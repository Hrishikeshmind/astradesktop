# ASTRA-CAP-051 — Native local profiles and switcher
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 51

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-051` |
| Slug | `native-local-profiles-and-switcher` |
| Title | Native local profiles and switcher |
| Canonical group | Identity & Separation |
| Secondary groups | `Developer`, `Corporate` |
| Audiences | `NORM`, `DEV`, `CORP` |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 51). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- REJECT any second profile system (ASTRA-EXCLUSION-003); upstream dual profile-system risk.
...
| canonical_index | 51 |
| canonical_title | Native local profiles and switcher |
...
| notes | 1:1 with matrix block E2 (Local multi-profile). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Toolkit SelectableProfileService |
| UX owner | Astra hooks over native profiles UI |
...
- `engine/toolkit/profile/ (SelectableProfileService)`

## Proposed Astra / Zen entrypoints

Astra UI hooks over SelectableProfileService; reject second profile system.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/browser/base/content/browser-profiles-js.patch (present)`
- `src/toolkit/profile/nsToolkitProfileService-cpp.patch (present)`
- `prefs/firefox/browser.yaml (browser.profiles.enabled verified)`
...
| state_refs | `ASTRA-STATE-011` |

- `ASTRA-STATE-011` — owner: **Two upstream systems:** SelectableProfileService (SQLite) + ToolkitProfileService (`profiles.ini`) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))

## Conflicts and resolution rules

- `ASTRA-CONFLICT-003` — Profiles vs Spaces (map #3, S3; + upstream dual profile systems). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
- `ASTRA-CONFLICT-005` — Migration vs profile creation (map #5, S2). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
`NATIVE_PREF` `browser.profiles.enabled` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `browser.profiles.enabled` and/or remove Astra discoverability surface. Do not delete native backend.
