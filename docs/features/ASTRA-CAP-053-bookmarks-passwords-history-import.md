# ASTRA-CAP-053 — Bookmarks, passwords and history import resources
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 53

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-053` |
| Slug | `bookmarks-passwords-history-import` |
| Title | Bookmarks, passwords and history import resources |
| Canonical group | Identity & Separation |
| Secondary groups | `Everyday Productivity` |
| Audiences | `NORM` |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 53). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Import lands in native profile stores; no folder/DB copying by Astra.
...
| canonical_index | 53 |
| canonical_title | Bookmarks, passwords and history import resources |
...
| notes | Sub-aspect of matrix block E3 (per-resource import). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox MigrationUtils |
| UX owner | Native (Firefox) via Migration Center entrypoints |
...
- `Firefox MigrationUtils resource importers`

## Proposed Astra / Zen entrypoints

None required for import backends (NATIVE_ONLY); entry via Migration Center.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/common/modules/AstraMigrationCenter.mjs (present)`
...
| state_refs | `ASTRA-STATE-001`, `ASTRA-STATE-011` |

- `ASTRA-STATE-001` — owner: Firefox Places (`places.sqlite`) (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
