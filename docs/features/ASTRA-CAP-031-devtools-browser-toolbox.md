# ASTRA-CAP-031 — Browser Toolbox / chrome debugging
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 31

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-031` |
| Slug | `devtools-browser-toolbox` |
| Title | Browser Toolbox / chrome debugging |
...
Primary classification `ENABLE_AFTER_TEST` is taken exactly from the human-approved 62-item catalog (index 31). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Off by default; policy-controllable via DisableDeveloperTools.
...
| canonical_index | 31 |
| canonical_title | Browser Toolbox / chrome debugging |
...
| notes | Canonical slice of matrix block B2 (Browser Toolbox). |
...
| UX owner | Astra entrypoint over native Browser Toolbox |
...
- `engine/devtools/client/framework/browser-toolbox/`

## Proposed Astra / Zen entrypoints

Proposed advanced-tier entrypoint; off by default; policy-controllable.
