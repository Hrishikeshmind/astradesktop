...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 20

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-020` |
| Slug | `devtools-web-console` |
| Title | Web Console |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 20). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.
...
| notes | Canonical slice of matrix block B1 (DevTools console). |
...
Proposed Developer Hub launcher to native Web Console.
