# ASTRA-CAP-018 — Zoom, contrast, reduced motion and keyboard navigation
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 18

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-018` |
| Slug | `zoom-contrast-reduced-motion-keyboard-nav` |
| Title | Zoom, contrast, reduced motion and keyboard navigation |
| Canonical group | Language & Access |
...
Primary classification `ASTRA_UX_WRAPPER` is taken exactly from the human-approved 62-item catalog (index 18). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Custom Astra/Zen controls must meet ARIA baseline (a11y audit).
...
| canonical_index | 18 |
| canonical_title | Zoom, contrast, reduced motion and keyboard navigation |
| canonical_group | Language & Access |
...
| notes | 1:1 with matrix block A8 (Zoom/Accessibility/keyboard). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Gecko/Toolkit platform accessibility |
| UX owner | Astra accessibility entrypoint over platform a11y |
...
- `engine/accessible/`
- `engine/layout/`
- `engine/toolkit/themes/shared/design-system/`

## Proposed Astra / Zen entrypoints

Proposed accessibility entrypoint shell over platform zoom/contrast/reduced-motion; ARIA audit of custom panels.
...
| state_refs | `ASTRA-STATE-005`, `ASTRA-STATE-027` |

- `ASTRA-STATE-005` — owner: Firefox content-prefs (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
- `ASTRA-STATE-027` — owner: Gecko/Toolkit platform + design tokens (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))

## Conflicts and resolution rules

- `ASTRA-CONFLICT-017` — Compact Mode vs AI / sidebar panels (map #17, S3). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
...
- `ASTRA-CONFLICT-026` — Accessibility semantics across XUL and HTML controls (map #26, S2/S1-GOV). Status: RESOLVED_BY_DESIGN (architecture constraint only; not runtime proof). See [astra-conflict-resolution.md](../architecture/astra-conflict-resolution.md).
