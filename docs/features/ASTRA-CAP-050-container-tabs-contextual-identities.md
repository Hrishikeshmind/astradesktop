# ASTRA-CAP-050 — Container tabs / contextual identities
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 50

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-050` |
| Slug | `container-tabs-contextual-identities` |
| Title | Container tabs / contextual identities |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 50). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Containers organize identity; not a substitute for profiles or Spaces.
...
| canonical_index | 50 |
| canonical_title | Container tabs / contextual identities |
...
| notes | Sub-aspect of matrix block C5 (containers/work separation). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox ContextualIdentityService |
| UX owner | Astra/Zen persona integration over native containers |
...
- `engine/toolkit/components/contextualidentity/`

## Proposed Astra / Zen entrypoints

Persona/Work Space integration over native containers (no second identity store).

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/privatefox/privacy.yaml (privacy.userContext.enabled verified)`
...
| state_refs | `ASTRA-STATE-010` |

- `ASTRA-STATE-010` — owner: Firefox ContextualIdentityService (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
...
`NATIVE_PREF` `privacy.userContext.enabled` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
...
Revert/disable via verified native pref `privacy.userContext.enabled` and/or remove Astra discoverability surface. Do not delete native backend.
