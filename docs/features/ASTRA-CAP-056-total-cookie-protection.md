# ASTRA-CAP-056 — Total Cookie Protection
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 56

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-056` |
| Slug | `total-cookie-protection` |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 56). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- cookieBehavior=5 (TCP); keep firstparty.isolate off to not break login.
...
| canonical_index | 56 |
| canonical_title | Total Cookie Protection |
...
| notes | Canonical slice of matrix block G1 (TCP/cookie behavior). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox netwerk (cookie behavior) |
...
- `engine/netwerk/ (cookieBehavior / TCP)`

## Proposed Astra / Zen entrypoints

Suraksha reads TCP/cookie state; Firefox owns backend.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/privatefox/privacy.yaml (network.cookie.cookieBehavior verified)`
...
`NATIVE_PREF` `network.cookie.cookieBehavior` (status: VERIFIED) — verified present in worktree prefs/ where applicable.
