...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 4

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-004` |
| Slug | `pdf-signatures` |
| Title | PDF signatures |
| Canonical group | Student & Reading |
| Secondary groups | _none_ |
| Audiences | `CORP`, `GOV`, `STU` |
...
Primary classification `EXPOSE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 4). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.
...
| canonical_index | 4 |
| canonical_title | PDF signatures |
...
| notes | Canonical slice of matrix block A1 (signature editor). |
...
| UX owner | Astra entrypoint over native signature editor |
...
Proposed student/PDF signature discoverability entrypoint over native signature editor.
