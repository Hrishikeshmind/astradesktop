# ASTRA-CAP-033 — EnterprisePolicies engine
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 33

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-033` |
| Slug | `enterprise-policies-engine` |
| Title | EnterprisePolicies engine |
| Canonical group | Corporate |
| Secondary groups | _none_ |
| Audiences | `CORP`, `GOV` |
| Classification | `USE_NATIVE` |
| Supporting classifications | `INTEGRATE`, `ASTRA_UX_WRAPPER` |
...
Primary classification `USE_NATIVE` is taken exactly from the human-approved 62-item catalog (index 33). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Enterprise policy must always win over persona presets.
...
| canonical_index | 33 |
| canonical_title | EnterprisePolicies engine |
...
| notes | 1:1 with matrix block C1 (EnterprisePolicies engine). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox EnterprisePolicies |
| UX owner | Native (Firefox); Astra managed status is separate (CAP-041) |
...
None required (NATIVE_ONLY). Sample policies.json is Batch 4 packaging, not a second engine.
