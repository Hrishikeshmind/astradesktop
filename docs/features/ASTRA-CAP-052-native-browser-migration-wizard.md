# ASTRA-CAP-052 — Native browser migration wizard
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 52

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-052` |
| Slug | `native-browser-migration-wizard` |
| Title | Native browser migration wizard |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 52). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- REJECT any second migration engine (ASTRA-EXCLUSION-004); thin wrapper over native wizard.
...
| canonical_index | 52 |
| canonical_title | Native browser migration wizard |
...
| notes | 1:1 with matrix block E3 (Migration engine). |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox MigrationWizard |
| UX owner | Astra Migration Center (UI shell only) |
...
- `Firefox MigrationWizard / MigrationUtils`

## Proposed Astra / Zen entrypoints

Astra Migration Center thin wrapper over native wizard.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `src/zen/common/modules/AstraMigrationCenter.mjs (present)`
- `src/zen/common/modules/AstraMigrationBootstrap.mjs (present)`
