# ASTRA-CAP-040 — Telemetry and crash-report policy controls
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 40

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-040` |
| Slug | `telemetry-and-crash-report-policy` |
| Title | Telemetry and crash-report policy controls |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 40). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Telemetry locked off; crash reporter not shipped -> verify controls hold.
...
| canonical_index | 40 |
| canonical_title | Telemetry and crash-report policy controls |
...
| notes | Sub-aspect of C1 (policy) and Part D (telemetry/crash control); crash reporter absent. |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox Telemetry (locked off) + policy engine |
| UX owner | Astra managed integration over native telemetry policy |
...
Current readiness `NEEDS_CLARIFICATION` is preserved from the registry. Existence of this feature document does **not** advance readiness to `DESIGN_READY_FOR_IMPLEMENTATION`.

## Upstream implementation locations

Evidence status for every `engine/**` path: **UNVERIFIED — UPSTREAM REVISION NOT PINNED**.

- `engine/toolkit/components/telemetry/`
- `engine/toolkit/crashreporter/ (not shipped)`

## Proposed Astra / Zen entrypoints

Proposed managed status for telemetry/crash controls; crashreporter not shipped.

### Related overlay / prefs paths (worktree presence only; not E2+)

- `prefs/privatefox/privacy.yaml (present)`
...
| state_refs | `ASTRA-STATE-017`, `ASTRA-STATE-019` |
...
- `ASTRA-STATE-019` — owner: Firefox Telemetry (locked off); crash reporter **not shipped** (canonical text only in [astra-state-ownership.md](../architecture/astra-state-ownership.md))
...
`POLICY_CONTROLLED` `DisableTelemetry` (status: PROPOSED).
...
Use enterprise policy controls (`DisableTelemetry`) and/or remove Astra status/entrypoint surface.
