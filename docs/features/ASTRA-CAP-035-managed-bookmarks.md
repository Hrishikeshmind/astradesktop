# ASTRA-CAP-035 — Managed bookmarks
...
> Canonical source: `HUMAN_APPROVED_ASTRA_62_CATALOG_2026-07-19` index 35

## Identity

| Field | Value |
|---|---|
| Stable ID | `ASTRA-CAP-035` |
| Slug | `managed-bookmarks` |
...
Primary classification `INTEGRATE` is taken exactly from the human-approved 62-item catalog (index 35). Supporting tags from the capability matrix are recorded as notes, not extra registry rows.

### Integration notes

- Delivered via EnterprisePolicies ManagedBookmarks; do not duplicate Places.
...
| canonical_index | 35 |
| canonical_title | Managed bookmarks |
...
| notes | Sub-aspect of matrix block C1 (policy engine); ManagedBookmarks policy. |

## Owners

| Role | Owner |
|---|---|
| Native owner | Firefox EnterprisePolicies + Places |
...
Proposed managed integration presenting ManagedBookmarks policy results; Places remains store.
...
| state_refs | `ASTRA-STATE-001`, `ASTRA-STATE-017` |
...
`POLICY_CONTROLLED` `ManagedBookmarks` (status: PROPOSED).
...
Use enterprise policy controls (`ManagedBookmarks`) and/or remove Astra status/entrypoint surface.
